...
export class MissionService {
    ...
    async getMissionRecommend(recommendMissionDto : RecommendMissionDto): Promise <any> {
        ...
        const response = await openai.createChatCompletion({
            model: "gpt-3.5-turbo",
            messages: [
                { role: "system", content: "부모가 자식한테 시키는 일이고, 요청의 조건인 age, place, ability에 대해 주의해서 간단명료하게 대답해줘" },
                { role: "user", content: `너는 ${age}살 아이를 가지고 있는 부모님이야. 혼자 떨어져 있는 아이에게 ${place}에서 수행할 만한 참신한 미션 5가지만 추천해줘. 아이의 ${ability} 능력을 향상시키는 일이었으면 좋겠어. 다음 내용은 꼭 지켜줘: 미션내용은 항상 한 줄로, 각각의 미션 내용은 모두 'OO미션: -하기'와 같은 명사형으로 끝내고, 숫자를 미션명에 포함하지 마.` },
            ],
        });
